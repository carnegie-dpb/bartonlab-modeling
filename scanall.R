...
scanall = function(schema, condition, minR2=0.60, minExpr=10, minEtap=1, minEtapHat=0.1, host="localhost") {
    ...
    fits = data.frame(id=character(),name=character(),group=character(),rhop0=numeric(),etap=numeric(),etap.hat=numeric(),gammap=numeric(),kappa=numeric(),logFCinf=numeric(),R2=numeric(),check.rows=T)
            ...
            if (!is.null(fit.E) && !is.null(fit.L)) {
                ...
                if (!is.nan(R2.L) && R2.L>minR2) {
                    ...
                    if (abs(etap)>minEtap && abs(etap.hat)>minEtapHat) {
                        print(paste(ids[i], name, "L", round(meanExpr,2), round(etap,2), round(etap.hat,2), round(gammap,2), round(kappa,2), round(logFCinf,2), round(R2.L,2)), quote=F)
                        fits = rbind(fits, data.frame(id=ids[i],name=name,group="L",rhop0=rhop0,etap=etap,etap.hat=etap.hat,gammap=gammap,kappa=kappa,logFCinf=logFCinf,R2=R2.L))
                    }
                    ...
                    etap.hat = etap*rhon0/rhop0
                    if (abs(etap)>minEtap && abs(etap.hat)>minEtapHat) {
                        print(paste(ids[i], name, "E", round(meanExpr,2), round(etap,2), round(etap.hat,2), round(gammap,2), round(kappa,2), round(logFCinf,2), round(R2.E,2)), quote=F)
                        fits = rbind(fits, data.frame(id=ids[i],name=name,group="E", rhop0=rhop0,etap=etap,etap.hat=etap.hat,gammap=gammap,kappa=kappa,logFCinf=logFCinf,R2=R2.E))
                    }
                }
            }
        }
